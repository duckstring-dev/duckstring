...
- **Mode/PK travel** via a `_trickle.json` **sidecar** in the published `data_dir` (a cross-Pond reader has no access to the producer's registry) + a registry meta table `_duckstring_trickle` (hidden from the publish set; `registry_tables`/`extract_schema` exclude `_duckstring_*` tables/columns and `__changelog` companions). Data-plane `export` is Trickle-aware via `dataplane.publish_plan` (validates non-Trickle tables, exempts Trickle ones, writes the sidecar) — call sites unchanged. **Cross-Catchment**: the sidecar travels in the draw zip (`routes/draw.py` + `poller.py`) so a drawn Trickle source resolves downstream; the draw still ships the *whole* changelog+main (correct — the consumer windows on its own side — incremental-transfer windowing is the remaining optimization).
- **Iceberg small-writes** (`iceberg_plane._append_commit`): an append history and a merge `__changelog` publish via Iceberg **append-commits** — one `_duckstring_f`-homogeneous data file per run (the run's `duckstring.f` snapshot stamp is the "last published f" cursor), so the window read prunes by manifest stats. The merge **main** and plain-Ripple output stay overwrite. Two Iceberg-compat invariants: `_duckstring_f` is exported under `SET TimeZone='UTC'` (pyiceberg rejects non-UTC tz) and `_duckstring_hash` is a **VARCHAR** (unsigned 64-bit overflows Iceberg signed-long stats) — keep both.
...
- **Deferred** (correctness never depends on them): the incremental-draw **window** (ship only the changelog window), and `pond.state_dir`. Two known gaps: the Iceberg changelog isn't trimmed on retention (over-retains space, reads stay correct), and the comprehensive-replay crash window noted in `plans/trickle.md`. Speced there.
