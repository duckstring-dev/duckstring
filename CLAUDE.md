...
- **Deferred** (correctness never depends on them): true small-write *file* output + Iceberg manifest pruning, the incremental-draw **window** (ship only the changelog window), and `pond.state_dir`. Speced in `plans/trickle.md`.
