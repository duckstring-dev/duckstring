...
- **Order-dependent running values** (`acc.sum`, `acc.ema`, …) are a *different* operator: [`.along(col).accumulate(by, **acc.specs)`](../reference/python-api.md#order-dependent-scans--alongaccumulate) — a per-row scan (not a reduction) along a freshness-monotonic axis, finished by `.append()`. Separate `duckstring.acc` namespace; append-only.
