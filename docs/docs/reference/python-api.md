...
- Bootstrap / coverage-miss / changed-Ripple / over-`p` → comprehensive recompute diffed against the last-written main. The op set is closed — a join operand carrying its own `.filter()`/`.select()`/`.aggregate()`/`.sql()`, a missing merge key, an ambiguous join key, or a joined graph with no `.select` raises at build time.
